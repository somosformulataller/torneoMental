...
- **Aprobar pago / pagar retiro con mensaje al jugador** (2026-07-23, sin migración): al **aprobar un pago** (Compra de tickets) se abre un modal donde el admin puede, opcional, enviarle al **chat** del jugador una nota escrita, nota de voz y/o foto/PDF (componente `ChatComposeFields` + util `sendComposeToPlayer`, que reusa el chat existente y le enciende la campana), o marcar **"Solo aprobar"** (aprueba sin enviar nada). Cada compra pendiente tiene además un botón "💬 Nota" para escribirle sin aprobar. El modal de **pagar retiro** ganó un campo de nota opcional (además de referencia + comprobante) que también se le manda al chat. Verificado: aprobar con nota → ticket acreditado + nota en el chat; "solo aprobar" → acredita sin mensaje.
